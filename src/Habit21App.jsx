...
export default function Habit21App() {
  ...
  // ------------------------
  // UI (no nested page shell; the Dashboard provides the page shell)
  // ------------------------
  return (
    <>
      {/* Banner / Summary */}
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div className="p-4 rounded-2xl border border-neutral-800 bg-neutral-900/40">
            ...
            <div className="mt-2 text-xs text-neutral-400">
              Start: {habit.startDate} · End: {endDate}
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl border border-neutral-800 bg-neutral-900/40">
            ...
            <div className="mt-1 text-xs text-neutral-400">
              Consecutive days with ≥1 productive hour
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-neutral-400">
              {withinWindow ? currentDayLabel : "(outside 21-day window)"}
            </span>
          </div>
          <div className="flex gap-2 text-sm">
            <Badge
              colorClass="bg-emerald-500/20 text-emerald-300 border-emerald-600/40"
              label={`Productive: ${totals.productive}`}
            />
            <Badge
              colorClass="bg-neutral-500/20 text-neutral-300 border-neutral-600/40"
              label={`Neutral: ${totals.neutral}`}
            />
            <Badge
              colorClass="bg-rose-500/20 text-rose-300 border-rose-600/40"
              label={`Waste: ${totals.waste}`}
            />
          </div>
        </div>
      </section>
                ...
                >
                  ...
                  <div className="mt-1 text-sm line-clamp-2 min-h-[2rem]">
                    {entry ? (
                      ...
                      <span className="text-neutral-600">Log activity…</span>
                    )}
                  </div>
                  {entry && (
                    <button
        ...
        <div className="grid lg:grid-cols-2 gap-4">
          ...
          <div className="p-4 rounded-2xl border border-neutral-800 bg-neutral-900/40 lg:col-span-2">
            ...
            <div className="flex gap-3 text-sm">
              <Badge
                colorClass="bg-emerald-500/20 text-emerald-300 border-emerald-600/40"
                label={`Productive: ${overall.productive}`}
              />
              <Badge
                colorClass="bg-neutral-500/20 text-neutral-300 border-neutral-600/40"
                label={`Neutral: ${overall.neutral}`}
              />
              <Badge
                colorClass="bg-rose-500/20 text-rose-300 border-rose-600/40"
                label={`Waste: ${overall.waste}`}
              />
            </div>
          </div>
        </div>
      ...
      {editor && (
        <Modal
          ...
          title={`Log – ${selectedDate} · ${String(editor.hour).padStart(2, "0")}:00`}
        >
          <HourEditor
            value={editor}
            onChange={setEditor}
...
function Badge({ colorClass, label }) {
  return (
    <span className={`px-2.5 py-1 rounded-lg border text-xs ${colorClass}`}>
      {label}
    </span>
  );
}
